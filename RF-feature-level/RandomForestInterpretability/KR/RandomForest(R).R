...
dataset = 'KR'
position= 'G:\\华统\\MSI和MSS\\数据汇总8(基于7进行训练集测试集划分)\\KR'
...
train.MSI.Whole <- cbind(train.MSI.whole, label = rep('msi', 32674))
train.MSS.Whole <- cbind(train.MSS.whole, label = rep('mss', 63507))
test.MSI.Whole <- cbind(test.MSI.whole, label = rep('msi', 15448))
test.MSS.Whole <- cbind(test.MSS.whole, label = rep('mss', 27513))
...
save.path = 'G:\\华统\\MSI和MSS\\结果汇总7(跟随数据汇总)\\KR'
setwd(save.path)
