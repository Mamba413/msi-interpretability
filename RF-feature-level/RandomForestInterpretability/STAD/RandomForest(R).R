...
dataset = 'DX'
position= 'G:\\华统\\MSI和MSS\\数据汇总8(基于7进行训练集测试集划分)\\STAD'
...
train.MSI.Whole <- cbind(train.MSI.whole, label = rep('msi', nrow(train.MSI.whole)))
train.MSS.Whole <- cbind(train.MSS.whole, label = rep('mss', nrow(train.MSS.whole)))
test.MSI.Whole <- cbind(test.MSI.whole, label = rep('msi', nrow(test.MSI.whole)))
test.MSS.Whole <- cbind(test.MSS.whole, label = rep('mss', nrow(test.MSS.whole)))
...
path <- 'G:\\华统\\MSI和MSS\\结果汇总7(跟随数据汇总)\\STAD'
setwd(path)
